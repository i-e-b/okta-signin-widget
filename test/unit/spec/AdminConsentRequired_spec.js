define([
  ...
  'sandbox',
  'helpers/mocks/Util',
  'helpers/dom/AdminConsentRequiredForm',
  'helpers/util/Expect',
  'helpers/xhr/ADMIN_CONSENT_REQUIRED',
  'helpers/xhr/SUCCESS',
  'helpers/xhr/CANCEL'
], function (
  Okta, OktaAuth, LoginRouter, LoginUtil, $sandbox,
  Util, AdminConsentRequiredForm, Expect,
  resAdminConsentRequired, resSuccess, resCancel) {
  ...
  function setup (settings, res = resAdminConsentRequired) {
    ...
    const router = new LoginRouter(_.extend({
      el: $sandbox,
      baseUrl: baseUrl,
  ...
  Expect.describe('AdminConsentRequired', function () {
    ...
    itp('cancel button click cancels the current stateToken and calls the cancel function', function () {
      ...
      return setup({ adminConsent: { cancel } }).then(function (test) {
